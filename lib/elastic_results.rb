...
module ElasticResults
  ...
  def self.unsafe_index(index, type, payload)
    request = Net::HTTP::Post.new("#{ElasticResults.es_path}/#{index}/#{type}", 'Content-Type' => 'application/json')
  ...
  def self.ensure_index_exists(index, type)
    ...
    request = Net::HTTP::Put.new("#{ElasticResults.es_path}/#{index}", 'Content-Type' => 'application/json')
